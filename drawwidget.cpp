...
DrawWidget::DrawWidget(QWidget *parent) : QWidget(parent)
{
    ...
    _pointPen = QPen(QColor(10, 10, 255), 3, Qt::SolidLine, Qt::RoundCap);
    _hullPointPen = QPen(QColor(255, 10, 10), 4, Qt::SolidLine, Qt::RoundCap);
    _axisPen = QPen(QColor(60, 60, 60), 1);
    _specialPointPen = QPen(QColor(255, 10, 10), 6, Qt::SolidLine, Qt::RoundCap);
...
void DrawWidget::drawPoint(QPainter &painter, QPoint point)
{
    ...
    QPoint screenPoint = toScreenCoords(point);
    int radius = 2; // Fixed radius of 5 pixels
...
void DrawWidget::drawHull(QPainter &painter)
{
    if (_convexHull.size() > 0)
    {
        if (_convexHull.size() == 1)
        {
            ...
            int radius = _hullPointPen.width() / 2;
            painter.drawEllipse(first, radius, radius); // Changed to circle
        ...
        painter.drawText(toScreenCoords(_points[_convexHull.last()]).x() + 8,
                         toScreenCoords(_points[_convexHull.last()]).y() + 8,
                         QString::number(_convexHull.size() - 1));
    }
}
...
void DrawWidget::runJarvis()
{
    ...
    if (_points.size() > 0)
    {
        ...
        connect(_jarvis, SIGNAL(highlightLine(int)), this, SIGNAL(highlightLine(int))); // Critical connection
...
void DrawWidget::runGraham()
{
    ...
    if (_points.size() > 0)
    {
        ...
        connect(_graham, SIGNAL(highlightLine(int)), this, SIGNAL(highlightLine(int))); // Critical connection
...
void DrawWidget::runDivideAndConquer()
{
    ...
    if (_points.size() > 0)
    {
        ...
        connect(_divideAndConquer, SIGNAL(highlightLine(int)), this, SIGNAL(highlightLine(int))); // Critical connection
...
void DrawWidget::paintEvent(QPaintEvent *event)
{
    ...
    int radius = _specialPointPen.width() / 2;
    painter.drawEllipse(toScreenCoords(_specialPoint), radius, radius);
